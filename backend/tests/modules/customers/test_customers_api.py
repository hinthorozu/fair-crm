...
def test_list_customers_custom_page_size_and_page_two(client, auth_headers):
    for index in range(3):
        client.post(
            "/api/v1/customers",
            json={"display_name": f"Paged Customer {index}"},
            headers=auth_headers,
        )

    page_one = client.get("/api/v1/customers?page=1&page_size=2", headers=auth_headers)
    ...
    page_two = client.get("/api/v1/customers?page=2&page_size=2", headers=auth_headers)
