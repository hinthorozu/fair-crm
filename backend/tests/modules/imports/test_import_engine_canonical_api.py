"""Import engine behavior through the canonical wizard API lifecycle."""
...
def _rows(client, auth_headers, batch_id: str) -> list[dict]:
    response = client.get(f"{IMPORTS_BASE}/{batch_id}/rows", headers=auth_headers)
    assert response.status_code == 200
    return response.json()["items"]


def test_canonical_import_creates_analyzed_batch(client, auth_headers):
    response = create_analyzed_import(
        client,
        auth_headers,
        ["Firma Adı", "E-posta"],
    ...
    assert data["status"] == "decision_required"


def test_company_name_only_row_is_valid(client, auth_headers):
    response = create_analyzed_import(client, auth_headers, ["Firma Adı"], [["Solo Firma A.Ş."]])
    rows = _rows(client, auth_headers, response.json()["id"])
    assert len(rows) == 1
...
def test_blank_rows_are_skipped(client, auth_headers):
    ...
    rows = _rows(client, auth_headers, data["id"])
    assert len(rows) == 2
...
def test_fuzzy_customer_match(client, auth_headers, db_session, organization_id):
    ...
    row = _rows(client, auth_headers, response.json()["id"])[0]
    assert row["match_confidence"] >= 85
    assert row["match_reason"] in ("exact_normalized_match", "fuzzy_name_candidate")
    expected_status = (
        "ready_to_update"
        if row["match_reason"] == "exact_normalized_match"
...
def test_header_alias_mapping(client, auth_headers):
    ...
    row = _rows(client, auth_headers, response.json()["id"])[0]
    assert row["normalized_data_json"]["company_name"] == "Beta A.Ş."
    assert row["normalized_data_json"]["contact_first_name"] == "Ali"
...
def test_invalid_email_row_is_valid_in_company_name_only_mode(client, auth_headers):
    ...
    row = _rows(client, auth_headers, response.json()["id"])[0]
    assert row["status"] == "ready_to_create"
    assert row["match_reason"] == "no_match"
...
def test_exact_customer_match_is_fair_aware(client, auth_headers, db_session, organization_id):
    ...
    assert row["match_reason"] == "exact_normalized_match"
...
def test_decision_create_new(client, auth_headers):
    response = create_analyzed_import(client, auth_headers, ["Firma Adı"], [["New Co"]])
    batch_id = response.json()["id"]
    row_id = _rows(client, auth_headers, batch_id)[0]["id"]
    ...
    batch = client.get(f"{IMPORTS_BASE}/{batch_id}", headers=auth_headers).json()
...
def test_decision_skip(client, auth_headers):
    response = create_analyzed_import(client, auth_headers, ["Firma Adı"], [["Skip Co"]])
    batch_id = response.json()["id"]
    row_id = _rows(client, auth_headers, batch_id)[0]["id"]
    decision = client.patch(
        f"{IMPORTS_BASE}/{batch_id}/rows/{row_id}/decision",
...
def test_apply_creates_customer_with_social_urls_from_excel(client, auth_headers):
    ...
    batch_id = response.json()["id"]
    rows = _rows(client, auth_headers, batch_id)
    assert rows[0]["status"] == "ready_to_create"
    normalized = rows[0]["normalized_data_json"]
    ...
    customer = next(
        item
        for item in customers.json()["items"]
        if item["display_name"] == "Social Excel Co"
    )
...
def test_apply_does_not_overwrite_existing_filled_fields(
    client,
    auth_headers,
    db_session,
    organization_id,
...
def test_invalid_rows_are_not_applied(client, auth_headers):
    ...
    client.patch(
        f"{IMPORTS_BASE}/{batch_id}/rows/{valid_row['id']}/decision",
        headers=auth_headers,
        json={"decision": "create_new"},
    )
