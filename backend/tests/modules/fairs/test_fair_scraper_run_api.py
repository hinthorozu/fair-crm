...
def test_run_fair_scraper_starts_run_and_completes_with_mock(client, auth_headers, db_session, organization_id):
...
def test_list_scraper_runs_filter_by_fair_id(client, auth_headers, db_session):
    ...
    response = client.get(f"/api/v1/scraper/runs?fair_id={fair_id}")
