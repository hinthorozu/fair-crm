...
from app.modules.quote_templates.api.dependencies import (
    require_create_permission, require_read_permission, require_update_permission,
)
...
from app.modules.quote_templates.api.schemas import (
    LogoUploadResponse, QuoteTemplateListResponse, QuoteTemplateResponse, QuoteTemplateWriteRequest,
)
...
def _find(db: Session, organization_id: UUID, template_id: UUID) -> QuoteTemplateModel:
    item = db.scalars(select(QuoteTemplateModel).where(
...
def list_quote_templates(auth: AuthContext = Depends(require_read_permission), db: Session = Depends(get_db)):
    templates = db.scalars(select(QuoteTemplateModel).where(
...
def create_quote_template(
    ...
    template = QuoteTemplateModel(organization_id=auth.organization_id, name=body.name.strip(), created_at=now, updated_at=now)
    db.add(template); db.flush()
    version = QuoteTemplateVersionModel(template_id=template.id, version_number=1, logo_url=body.logo_url, source_code=body.source_code, created_at=now, created_by=auth.user_id)
    db.add(version); db.flush()
    template.current_version_id = version.id
    db.commit(); db.refresh(template)
    response = _response(template, version)
    audit.record_event(
        organization_id=auth.organization_id, access_token=credentials.credentials if credentials else "dev-bypass",
        action="quote_template.created", resource_type="quote_template", resource_id=str(template.id),
...
def update_quote_template(
    ...
    now = datetime.now(timezone.utc)
    next_number = (db.scalar(select(func.max(QuoteTemplateVersionModel.version_number)).where(QuoteTemplateVersionModel.template_id == template.id)) or 0) + 1
    version = QuoteTemplateVersionModel(template_id=template.id, version_number=next_number, logo_url=body.logo_url, source_code=body.source_code, created_at=now, created_by=auth.user_id)
    db.add(version); db.flush()
    template.name = body.name.strip(); template.current_version_id = version.id; template.updated_at = now
    db.commit(); db.refresh(template)
    response = _response(template, version)
    audit.record_event(
        organization_id=auth.organization_id, access_token=credentials.credentials if credentials else "dev-bypass",
        action="quote_template.updated", resource_type="quote_template", resource_id=str(template.id),
        old_values=old_values, new_values={"name": template.name, "version_number": next_number},
    )
    return response


@router.post("/logo", response_model=LogoUploadResponse)
async def upload_quote_logo(file: UploadFile = File(...), auth: AuthContext = Depends(require_create_permission)):
    ...
    organization_dir = _LOGO_DIR / str(auth.organization_id)
