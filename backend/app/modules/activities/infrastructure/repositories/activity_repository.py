...
class SqlAlchemyActivityRepository:
    ...
    def exists_fair_bulk_email_outbox(self, organization_id: UUID, outbox_id: UUID) -> bool:
        """Compatibility: true if any attempt activity exists for this outbox."""
    ...
    def exists_fair_bulk_email_outbox_attempt(
        ...
        for model in models:
            ...
            if metadata.get("outbox_id") != outbox_id_text:
                continue
            stored_attempt = metadata.get("send_attempt")
            if stored_attempt is None:
                # Legacy rows (pre-attempt unique): treat as attempt 1
                if send_attempt == 1:
                    return True
                continue
            if str(stored_attempt) == attempt_text:
                return True
        return False
