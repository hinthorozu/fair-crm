...
def resolve_package_path(
    ...
    *,
    ...
    candidate = (resolved_execution / f"{package_id}.zip").resolve()
...
def write_immutable_package(
    *,
    ...
    try:
        ...
        generated_digest = _file_digest(temporary_path)
...
def resolve_stored_locator(locator: str, *, storage_root: Path | None = None) -> Path:
    ...
    if resolved.is_symlink():
        raise ClosurePackageStorageError("Closure package locator cannot resolve to a symlink")
    return resolved
...
def verify_package(
    *,
    ...
    if _file_digest(path) != expected_package_digest:
    ...
    try:
        with ZipFile(path, "r") as archive:
            ...
            if sha256(canonical_manifest).hexdigest() != expected_manifest_digest:
            ...
            for member in expected_members:
                ...
                if sha256(payload).hexdigest() != digest or len(payload) != byte_size:
