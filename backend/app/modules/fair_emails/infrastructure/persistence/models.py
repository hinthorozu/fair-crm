...
class FairEmailBatchModel(Base):
    ...
    fair_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), nullable=False, index=True)
...
class FairEmailOutboxModel(Base):
    ...
    participation_id: Mapped[UUID] = mapped_column(Uuid(as_uuid=True), nullable=False)
