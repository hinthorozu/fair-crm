...
import {
  ...
  COST_PRODUCT_UPDATE,
  COST_PRODUCT_VIEW,
  getGrantedCostCatalogPermissions,
} from "../permissions/costCatalogPermissions";
import type {
  CostCategory,
  CostCategoryOption,
  CostCategoryPayload,
...
export function CostCatalogPage() {
  ...
  const [categoryEditing, setCategoryEditing] = React.useState<
    CostCategory | null | undefined
  >(undefined);
  const [productEditing, setProductEditing] = React.useState<CostProduct | null | undefined>(
    undefined,
  );
  const [categoryValues, setCategoryValues] =
    React.useState<CostCategoryPayload>(emptyCategory);
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      setCategoryOptions(optionResult.items);
    } catch {
      setError("Maliyet kataloğu yüklenemedi.");
    } finally {
      setLoading(false);
    }
  }, [canCategoryView, canLoadProductCategoryOptions, canProductView]);
  ...
  const saveCategory = async (event: React.FormEvent) => {
    ...
    const isUpdate = Boolean(categoryEditing);
    if ((isUpdate && !canCategoryUpdate) || (!isUpdate && !canCategoryCreate)) return;
    setSaving(true);
    setError(null);
    try {
      if (categoryEditing) {
        await updateCostCategory(categoryEditing.id, categoryValues);
      } else {
        await createCostCategory(categoryValues);
      }
  ...
  const saveProduct = async (event: React.FormEvent) => {
    event.preventDefault();
    const isUpdate = Boolean(productEditing);
    if ((isUpdate && !canProductUpdate) || (!isUpdate && !canProductCreate)) return;
    setSaving(true);
    setError(null);
    try {
      if (productEditing) {
        await updateCostProduct(productEditing.id, productValues);
      } else {
        await createCostProduct(productValues);
      }
  ...
  const removeCategory = async (item: CostCategory) => {
    if (!canCategoryDelete) return;
    if (!window.confirm(`“${item.name}” kategorisi silinsin mi?`)) return;
    try {
      await deleteCostCategory(item.id);
      await load();
    } catch (err) {
  ...
  const removeProduct = async (item: CostProduct) => {
    if (!canProductDelete) return;
    if (!window.confirm(`“${item.name}” ürünü silinsin mi?`)) return;
    try {
      await deleteCostProduct(item.id);
      await load();
    } catch (err) {
  ...
  const productColumns: UniversalDataTableColumn<CostProduct>[] = [
    ...
    { key: "unit", title: "Birim", render: (item) => item.unit },
    {
      ...
      sortable: true,
      render: (item) =>
        `${Number(item.unit_price).toLocaleString("tr-TR", { maximumFractionDigits: 4 })} ${item.currency}`,
    },
    {
      key: "tl",
      title: "TL Karşılığı",
      render: (item) =>
        item.currency === "TL"
          ? `${Number(item.unit_price).toLocaleString("tr-TR", { maximumFractionDigits: 4 })} TL`
          : "Kur bekleniyor",
    },
    {
      key: "actions",
  ...
  return (
    <PageShell className="cost-catalog-page">
      <PageHeader
        title="Maliyet Kataloğu"
        subtitle="Organizasyonunuza ait maliyet kategorilerini ve ürünlerini yönetin"
      />
      {error ? <Banner variant="error">{error}</Banner> : null}
      {products.some((item) => item.currency === "USD") ? (
        <Banner variant="info">
          USD/TL kur sağlayıcısı karar kaydında henüz belirlenmediği için USD ürünlerin TL
          karşılığı bu sürümde hesaplanmıyor.
        </Banner>
      ) : null}

      {canAccessCategorySection ? (
        <section>
          <PageHeader
            title="Kategoriler"
            actions={
            ...
            <UniversalDataTable
              ...
              emptyState={
                <EmptyState
                  ...
                  description="İlk maliyet kategorisini oluşturun."
                />
              }
            />
          ) : (
            <Banner variant="info">
              Kategori listesini görüntülemek için kategori okuma yetkisi gerekir.
            </Banner>
          )}
        </section>
      ) : null}

      {canAccessProductSection ? (
        <section>
          <PageHeader
            title="Ürünler"
            actions={
          ...
          {canProductView ? (
            <UniversalDataTable
              ...
              onRetry={() => void load()}
              emptyState={
                <EmptyState title="Henüz ürün yok" description="İlk maliyet ürününü oluşturun." />
              }
            />
          ) : (
            <Banner variant="info">
              Ürün listesini görüntülemek için ürün okuma yetkisi gerekir.
            </Banner>
          )}
        </section>
      ) : null}

      {categoryEditing !== undefined ? (
        <FormModal
        ...
        >
          <form className="crm-form" onSubmit={saveCategory}>
              ...
              <TextInput
                ...
                onChange={(e) => {
                  ...
                  setCategoryValues((current) => ({
                    ...current,
                    name,
                    slug: categorySlugTouched ? current.slug : toSlug(name),
                  }));
                }}
                required
              />
            </FormField>
            <FormField label="Slug" htmlFor="cost-category-slug">
              <TextInput
                id="cost-category-slug"
                value={categoryValues.slug}
                onChange={(e) => {
                  setCategorySlugTouched(true);
                  setCategoryValues((current) => ({
                    ...current,
            ...
            <FormField label="Açıklama" htmlFor="cost-category-description">
              <TextareaInput
                id="cost-category-description"
          ...
          <form className="crm-form" onSubmit={saveProduct}>
              ...
              <TextInput
                ...
                onChange={(e) => {
                  ...
                  setProductValues((current) => ({
                    ...current,
                    name,
                    slug: productSlugTouched ? current.slug : toSlug(name),
                  }));
                }}
                required
              />
            </FormField>
            <FormField label="Slug" htmlFor="cost-product-slug">
              <TextInput
                id="cost-product-slug"
            ...
            <FormField label="Birim fiyat" htmlFor="cost-product-price">
              <TextInput
                ...
                onChange={(e) =>
                  setProductValues({ ...productValues, unit_price: e.target.value })
                }
                required
              />
            </FormField>
            <FormField label="Para birimi" htmlFor="cost-product-currency">
              <select
                ...
                onChange={(e) =>
                  setProductValues({
                    ...productValues,
                    currency: e.target.value as CostCurrency,
                  })
                }
              >
