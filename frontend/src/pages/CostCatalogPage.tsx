...
export function CostCatalogPage() {
  ...
  const productColumns: UniversalDataTableColumn<CostProduct>[] = [
    ...
    { key: "actions", title: "İşlemler", render: (item) => <div className="table-actions">{canProductUpdate ? <button className="btn secondary" type="button" onClick={() => { setProductValues({ category_id: item.category_id, name: item.name, slug: item.slug, unit: item.unit, unit_price: item.unit_price, currency: item.currency }); setProductEditing(item); }}>Düzenle</button> : null}{canProductDelete ? <button className="btn danger" type="button" onClick={() => void removeProduct(item)}>Sil</button> : null}</div> },
  ];
